...
// Same label either way; logged-out visitors must log in before publishing
const ctaButtonProps = computed(() => ({
  ...
  ...(bookstoreApiStore.isAuthenticated
    ...
    : {
        loading: isAuthenticating.value,
        onClick: () => bookstoreApiStore.openLoginPanel(),
      }),
}))

const stats = computed(() => [
  { value: '1,000+', label: $t('about.stats_books') },
  { value: '200,000+', label: $t('about.stats_users') },
  { value: '90%*', label: $t('about.stats_revenue') },
])
